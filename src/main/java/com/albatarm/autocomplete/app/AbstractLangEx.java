...
import com.albatarm.autocomplete.MyAutoCompleteProvider;
import com.albatarm.autocomplete.Scanner;

public class AbstractLangEx<T extends Lexer> implements LangEx {
	
	private static final Logger LOG = LogManager.getLogger(AbstractLangEx.class);
    ...
    protected AbstractLangEx(String grammarFile, String tokensFile, Set<Integer> separators, String rootRule, String[] tokenNames, Function<CharStream, T> lexerFactory) {
    	this(grammarFile, null, tokensFile, separators, rootRule, tokenNames, lexerFactory);
    }
    
    protected AbstractLangEx(String grammarFile, String importDir, String tokensFile, Set<Integer> separators, String rootRule, String[] tokenNames, Function<CharStream, T> lexerFactory) {
    ...
    public MyAutoCompleteProvider<T> create(String source, Caret caret) {
    	ANTLRInputStream input = new ANTLRInputStream(source);
        T lexer = lexerFactory.apply(input);
        Scanner<T> scanner = new Scanner<>(lexer, separators::contains);
        LOG.debug(() -> scanner.stream().map(Object::toString).collect(Collectors.joining(" ")));
        return new MyAutoCompleteProvider<T>(completer, Arrays.asList(tokenNames), scanner, rootRule, caret);
    }
